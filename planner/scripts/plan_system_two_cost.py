...
class PCTPlannerSystemNode(Node):
    def __init__(self):
        ...
        self.fixed_traj = load_traj_from_csv("/home/unitree/navigation/PctPlanner/rsc/traj/cabin_path.csv")
        self.fixed_traj2 = load_traj_from_csv("/home/unitree/navigation/PctPlanner/rsc/traj/enter_path.csv")
        # 定义矩形区域 (x_min, x_max, y_min, y_max)
        # 当机器人在此区域内时使用 costmap1，否则使用 costmap2
        self.region_x_min = -38.5
        self.region_x_max = -28.3
        self.region_y_min = 3.2
        ...
        self.get_logger().info("Initializing planner with costmap2...")
        self.planner.update_tomogram_from_msg(msg)
        self.tomogram_received = True
        self.current_costmap = 2  # 记录当前使用的 costmap
        self.get_logger().info("Planner initialized with costmap2.")

        # 起点位姿：初始为 (0, 0, 0)
        self.start_pos = np.array([-38.9,-14.35,0.0], dtype=np.float32)
        # self.start_pos = np.array([-37.5818, 6.4524,0.0])
        # # 目标位姿：固定为 (1, 6, 0)
        # self.start_pos = np.array([39.2952, -5.03867,0.25], dtype=np.float32)
        self.goal_pos2 = np.array([-37.5818, 6.30241, 0.0], dtype=np.float32)
        # self.goal_pos2 = np.array([-42.6271, 1.01581,0.0],dtype=np.float32)
        self.goal_pos1 = np.array([-31.75,6.900,2.08], dtype=np.float32)
    ...
    def update_costmap_based_on_position(self, x, y):
        ...
        if desired_costmap != self.current_costmap:
            if desired_costmap == 1:
                self.get_logger().info(f'Switching to realtime path (inside region)')
                pickled_data = pickle.dumps(self.data_dict1)
                self.goal_pos = self.goal_pos1
                self.should_plan = False
