...
class TomogramPlanner(object):
    ...
    def _initialize_from_dict(self, data_dict):
        ...
        self.slice_heights = data_dict['slice_heights']
        print(f"加载的 slice_heights: {self.slice_heights}, 长度: {len(self.slice_heights)}")
    ...
    def update_tomogram_from_msg(self, msg):
    ...
    def initPlanner(self, trav, trav_gx, trav_gy, elev_g, elev_c):
        ...
        self.planner.init_map(
            35, 15, self.resolution, self.n_slice, 0.2,
    ...
    def plan(self, start_pos, end_pos, start_z=0, end_z=0):
        # Clamp the slice index to be within valid range
        print(f"输入的 start_z: {start_z}, end_z: {end_z}")
        start_slice_idx = np.clip(self.z2slice(start_z), 0, self.n_slice - 1).astype(np.int32)
        end_slice_idx = np.clip(self.z2slice(end_z), 0, self.n_slice - 1).astype(np.int32)

        self.start_idx[0] = start_slice_idx
        self.start_idx[1:] = self.pos2idx(start_pos)

        self.end_idx[0] = end_slice_idx
        self.end_idx[1:] = self.pos2idx(end_pos)

        print(f"Start grid index (slice, y, x): {self.start_idx}")
        print(f"End grid index (slice, y, x): {self.end_idx}")
